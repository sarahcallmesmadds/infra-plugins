...
check('pickup names the thread instead of inheriting the session cwd', () => {
  ...
  // A handoff picked up by its path passes the file, and the command reads
  // its Working directory line, which is equally never the session's; every
  // documented command has to be one of the two.
  const text = skill('pickup');
  ...
  for (const c of cmds) {
    assert.match(c, /--thread|--file "<the path>"/,
