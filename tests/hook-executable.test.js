...
check('the guard exits on a code that reaches the reader and blocks nothing', () => {
  ...
  assert.notStrictEqual(CODEX_EXIT, GUARD_EXIT,
    'the two branches exit the same code, so one of the two hosts is being sent a code that buries '
    + 'its message: they surface a hook\'s words under opposite conditions');
});
