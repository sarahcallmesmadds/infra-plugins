...
- **If it matches a queue entry ID format** (YYYY-MM-DDTHH-MM-SS-{skill}, e.g. `2026-04-23T13-29-20-daily-brief`): read `~/.claude/skill-loop/queue/{id}.json` directly using the Read tool.

- **If it is a skill name** (e.g. `daily-brief`): run `ls ~/.claude/skill-loop/queue/` and scan all `.json` files. Find all entries where `skill == $ARGUMENTS` AND `status == "fix applied, watching"`.
  ...
  - If no matches: say "No 'fix applied, watching' entries found for '{skill}'. If the fix is in a different status, provide the full queue entry ID." Stop.
...
- If `status` is NOT `"fix applied, watching"`: say "This entry has status '{status}', not 'fix applied, watching'. /skill-revert-fix only reverts committed fixes. If you want to discard an in-progress attempt, use /skill-apply-fix and reply 'no' to the diff." Stop.
...
- If no `Committed:` note is found in `notes[]`: say "I can't find a commit hash in this queue entry's notes. The commit hash is normally stored by /skill-apply-fix after committing. You may need to find the commit manually with: `git -C {repo_root} log --oneline | head -10`" Stop.
...
The fix for {skill} will be reversed.
...
Determine `repo_root` by looking up the entry's `repo` field in `skillRoots` in
`~/.claude/skill-loop.config.json` and taking that root's `path`. With no config
file there is one root, `personal` at `~/.claude/skills`. If `repo` is
...
```
Done. The fix for {skill} has been reverted (undo commit: {revert-hash}).
Queue entry is back to Open.
The skill file is restored to its pre-fix state.
