# Skill Factory Queue Entry Schema

## Purpose

This document is the single source of truth for the queue entry format. Every skill and hook in the skill-factory system reads from and writes to this schema. When adding fields, bump `$schema_version` and update this document in the same commit.
...
```
~/.claude/skill-factory/queue/{timestamp}-{skill-slug}.json
```
...
```json
{
  ...
  "parent_id": null,
  "skill": "daily-brief",
  "skill_path": "/Users/fermat/.claude/skills/daily-brief/SKILL.md",
  "repo": "personal",
  "session_id": "00dc3f8e-941c-...",
  "session_cwd": "/Users/fermat/Projects/skill-factory",
...
```json
{
  ...
  "skill": "wrap",
  "skill_path": "/Users/fermat/.claude/skills/wrap/SKILL.md",
  "repo": "personal",
  "session_id": "00dc3f8e-941c-...",
  "session_cwd": "/Users/fermat/Projects/skill-factory",
...
| `skill_path` | string | ✓ | Absolute path to the skill's SKILL.md file. Used by Phase 3 to route commits to the right repo. | Example: `/Users/fermat/.claude/skills/daily-brief/SKILL.md` |
| `repo` | string | ✓ | Which GitHub repo owns this skill. Inferred from `skill_path` — see Repo Attribution Rule below. | `"personal"` or `"fermat-gtm"` or `"unknown"` |
...
| `correct_example` | string | ✓ | A concrete example of what the correct output would look like. | Quote Sarah's words when available. A rough example is fine. |
...
| `Won't Fix` | Sarah explicitly deferred or declined this correction. |
| `fix applied, watching` | Sarah approved the diff and the fix was committed. Fix is live but not yet confirmed in a real session. Sarah must manually close this to "Resolved" after using the skill. |
| `fix attempted / unresolved` | Sarah rejected the diff (said "no" or "retry" without resolving), or the Write tool failed mid-fix. The skill file is unchanged. |
...
| Type | Meaning |
|------|---------|
| `primary` | A directly-captured correction. Sarah invoked /capture or the Stop hook fired. |
...
- If `skill_path` starts with `/Users/fermat/.claude/skills/` → `repo: "personal"`
- If `skill_path` starts with `/Users/fermat/gtm/projects/claude-addons/` → `repo: "fermat-gtm"`
...
Then scan `~/.claude/skill-factory/queue/` for any existing entry with the same `dedup_key` created in the last 10 minutes. If one is found, skip the write and report the duplicate to Sarah instead of creating a second entry. This prevents the same correction being logged twice when both the Stop hook and `/capture` detect the same event.
...
**File location:** `~/.claude/skill-factory/pattern-flags.json`
...
| `repo` | string | ✓ | `"personal"` \| `"fermat-gtm"`. Copied from DEPS.json or inferred from `skill_path`. |
...
| `notes` | array | - | Append-only `{ts, text}` objects. Used by Tomas and Sarah for co-development notes. Never cleared. |

### Flag Status Enum

| Status | Meaning |
|--------|---------|
| `pending-review` | Flagged, waiting for Tomas co-development review. Default on creation. |
| `in-review` | Actively being discussed with Tomas for structural fix design. Set manually. |
...
| v4 | 2026-04-24 | Added pattern-flags.json schema (Phase 4 Intelligence Layer). pattern-flags.json lives at `~/.claude/skill-factory/pattern-flags.json`. Flag status enum: `pending-review` \| `in-review` \| `resolved`. Queue entry schema unchanged — pattern detection works from existing fields without modification. |
