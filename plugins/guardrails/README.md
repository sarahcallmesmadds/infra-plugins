...
The branch guard resolves explicit `cd` and `git -C` paths, including `~` and
relative paths. When a path is computed by the shell and cannot be resolved
from the command text, it checks the command's working directory instead. A
dynamic path that targets some other repository cannot be inspected reliably;
name that path explicitly when the distinction matters.
...
plugin governs nothing until you say so. Machine-specific entries do not belong
in a shared default, especially because the user registry replaces the shipped
...
Machine-specific entries belong there rather than in the shipped registry.
...
On macOS, `/tmp/` and `/private/tmp/` are safe defaults for disposable output.
The per-user directory under `/var/folders/` is not: its path differs by
machine. Add your resolved directory to `safeDeletePaths` if you routinely
delete disposable content there.
...
For `WebFetch`, the hook sees the processed result supplied to the model, not
the page's raw HTML. Content removed before that result reaches the hook cannot
be scanned.
