...
- **Atomic write is non-optional here.** The `.tmp` + parse-check + `mv` pattern applies to every JSON write that REPLACES an existing file. The one exception is `flag-issue`, which only ever creates new entries and says so in its own header. Never use a direct single-step write to pattern-flags.json.
